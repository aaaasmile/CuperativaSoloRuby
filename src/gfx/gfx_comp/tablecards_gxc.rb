...
class TablePlayedCardsGraph < ComponentBase
  ...
  def resize_with_info
    ...
    if info[:type_distr] == :linear
      ...
      @table_cards_gfx.each do |x|
        ...
        x.visible = false
        x.change_image(@gfx_res.get_cardsymbolimage_of(:coperto), :coperto)
      end
  ...
  def correct_table_card_onposition(position, single_gfx_card)
    case position
      when :nord
        ...
        single_gfx_card.cd_data[:position] = position
      when :sud
        single_gfx_card.pos_y += 10
        single_gfx_card.pos_x += 15
        single_gfx_card.cd_data[:position] = position 
      when :est
        single_gfx_card.pos_y += 0
        single_gfx_card.pos_x += 35
        single_gfx_card.cd_data[:position] = position
      when :ovest
        single_gfx_card.pos_y += 0
        single_gfx_card.pos_x -= 35
        ...
        @log.warn "Ignored card played from position #{position}: not implemented"
    end
  end
    ...
    if type == :card_played
      @cards_played_todisp.each do |gfx_card|
        ...
        if tb_gfx != nil
          ...
          if gfx_card.cd_data[:go_back] != true
            tb_gfx.visible = true
          end
        end
      end
  ...
  def rollback_card_played(position, card_lbl)
    #animation_is_terminated(:card_played)
    tb_gfx = find_position_card_on_table(position)
    if tb_gfx != nil
      tb_gfx.visible = false
    end
    @cards_played_todisp.each do |gfx_card|
       if gfx_card.lbl == card_lbl
         gfx_card.cd_data[:go_back] = true
       end
    end
    #p "table"
    #@table_cards_gfx.each{|e| p e.lbl; p e.visible}
    #p "played"
    #@cards_played_todisp.each{|e| p e.lbl; p e.visible}
    #p "taken"
    #@cards_taken_disp.each{|e| p e.lbl; p e.visible}
  end
  ...
  def card_taken_ontable2(player_taker, cards_taken)
    ...
    @mano_end_card_taken.each do |taked_lbl|
      ...
      if gfx_card
        ...
        gfx_card.visible = false
        gfx_card.change_image(@gfx_res.get_cardsymbolimage_of(:coperto), 
                                           :coperto)
      end
    end
  ...
  def initial_lbl_cards_on_table2(cards_lbl)
    @table_cards_gfx.each do |card_gfx| 
      ...
      card_gfx.change_image(@gfx_res.get_cardsymbolimage_of(:coperto), 
                                           :coperto)
    end
    ix = 0
  ...
  def onTimeoutAniCardPlayed1
    ...
    return if @cards_played_todisp.size == 0
