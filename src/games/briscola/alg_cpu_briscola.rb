...
class AlgCpuBriscola < AlgCpuPlayerBase
  ...
  def initialize(player, coregame, gfx_res)
    ...
    @players = nil
    # alg level 
    @level_alg = :master #:dummy, :predefined
    # briscola
    @briscola = nil
  ...
  def play_from_predef_stack
    card = @predef_stack.pop
    return card
  end
  
  def add_card_to_predef_stack(lbl_arr)
    @predef_stack.push(lbl_arr)
    @predef_stack.flatten
  end
