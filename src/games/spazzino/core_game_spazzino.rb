...
class CoreGameSpazzino < CoreGameBase
  ...
  def adjourn_points_manoend(player)
    ...
    @carte_prese[player.name].each do |card|
      ...
      if @deck_information.get_card_segno(card) == :S
        ...
        elsif   @deck_information.get_card_logical_symb(card) == :fan
          #onori += 1
  ...
  def alg_player_cardplayed_arr(player, arr_lbl_card)
    return if super(player, arr_lbl_card)
  ...
  def check_takentable_cards(lbl_card, card_taken)
    ...
    card_taken.each do |card|
      if @carte_on_table.index(card)
        taken_sum += @deck_information.get_card_rank(card)
      else
        # card not on table
        return false
      end
    end
    r1_rank = @deck_information.get_card_rank(lbl_card)
  ...
  def combi_of_sum(arr_cards_input, val)
    arr_cards = arr_cards_input.sort{ |x,y| @deck_information.get_card_rank(x) <=> @deck_information.get_card_rank(y)}
  ...
  def combi_line(list_curr, rest_arr, valtarget)
    ...
    rest_arr.each do |c2|
      ...
      sum_curr = list_new.inject(0) do |sum, x|
        sum += @deck_information.get_card_rank(x)
      end   
