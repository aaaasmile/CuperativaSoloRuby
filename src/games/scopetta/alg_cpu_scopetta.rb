...
class AlgCpuScopetta < AlgCpuSpazzino
  ...
  def card_allow_scopa?(card_item, pt_table)
    return @deck_info.get_card_rank(card_item) + pt_table <= 10
  end
  ...
  def calc_pt_card_arr(card_arr)
    ...
    card_arr.each do |card_item|
      ...
      sum += @deck_info.get_card_rank(card_item)
    end
    return sum
  end
  ...
  def card_is_denari?(card_tb)
    return @deck_info.get_card_segno(card_tb) == :D
  end
  ...
  def card_is_sette?(card_tb) 
    return @deck_info.get_card_logical_symb(card_tb) == :_sette 
  end
