...
class CoreGameTressette < CoreGameBase
  ...
  def card_could_be_played?(player_carte_in_mano, lbl_card, carte_gioc_mano_corr)
    ...
    if player_carte_in_mano.index(lbl_card)
      if carte_gioc_mano_corr.size > 0
        # we have suit
        suit = @suit_curr_mano
        card_played_segno = @deck_information.get_card_info(lbl_card)[:segno]
        if suit == card_played_segno
          return true
        else
           player_carte_in_mano.each do |carta_in_mano|
             card_inmano_segno = @deck_information.get_card_info(carta_in_mano)[:segno]
  ...
  def gui_new_match(players)
    ...
    @log.debug "First player is #{first_ix}"
    @mazziere_ix = player_ix_beforethis(players.size, first_ix)
    @log.debug "Mazziere is #{@mazziere_ix}"
  ...
  def new_giocata
    ...
    on_newgiocata_playersinitialized()
    #p @carte_prese
  ...
  def mano_end
    ...
    #p @carte_gioc_mano_corr
    str_info = "mano_end - carte giocate: "
    @carte_gioc_mano_corr.each{|x| str_info += "#{x[:card_lbl]} "}
    lbl_best,player_best_name =  vincitore_mano(@carte_gioc_mano_corr)
    player_best = @players_name_to_player[player_best_name]
    @mano_count += 1
    
    # dal giocatore che ha preso si passa a quello che ha il mazzo
    # la differenza si ha con i 4 giocatori dove rimangono 2 mazzi
    player_deck_coll = owner_name_of_deck(player_best_name)
    @last_playername_taken = player_deck_coll 
    
    carte_prese_mano = []
    @carte_gioc_mano_corr.each do |card_played_info|
      lbl_card = card_played_info[:card_lbl]
      carte_prese_mano << lbl_card
      @carte_prese[player_deck_coll] << lbl_card
    end 
    ...
    str_punti_presi = "tot #{punti_presi[:tot]}, pezze #{punti_presi[:pezze]}, assi #{punti_presi[:assi]}"
    @log.info "#{str_info}, vinta da #{player_best_name}, punti presi #{str_punti_presi}"
    ...
    # add points
    @points_curr_smazzata[player_deck_coll][:pezze] +=  punti_presi[:pezze]
    @points_curr_smazzata[player_deck_coll][:assi] +=  punti_presi[:assi]
    @points_curr_smazzata[player_deck_coll][:tot] =  @points_curr_smazzata[player_deck_coll][:assi] + @points_curr_smazzata[player_deck_coll][:pezze] / 3 
  ...
  def  card_is_asso?(card_lbl)
    card_info = @deck_information.get_card_info(card_lbl)
    return card_info[:points] == 3 ? true : false
  end
  
  def  card_is_pezza?(card_lbl)
    card_info = @deck_information.get_card_info(card_lbl)
    return card_info[:points] == 1 ? true : false
  end
