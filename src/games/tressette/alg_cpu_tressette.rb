...
class AlgCpuTressette < AlgCpuPlayerBase
  ...
  def initialize(player, coregame, game_wnd)
    ...
    # cards on deck
    @num_cards_on_deck = 0
    # predifined game, used on play_with_predifined level
    @action_queue = []
  ...
  def master_player_first
    ...
    [:B, :C, :D, :S].each do |suit|
      ...
      @cards_on_hand[suit].each do |card_lbl|
        info = @deck_info.get_card_info(card_lbl)
    ...
    @cards_on_hand[suit_max].each do |card_lbl|
      ...
      if @deck_info.get_card_info(card_lbl)[:rank] < 11
        pt_hands[card_lbl] += @deck_info.get_card_rank(card_lbl)
      else
        pt_hands[card_lbl] -= (@deck_info.get_card_rank(card_lbl) - 10 + 30)
      end
    end
  ...
  def master_play_second
    ...
    rank_cp = @deck_info.get_card_rank(card_played)
    points_cp = @deck_info.get_card_points(card_played)
    pt_hands = {}
    ...
    if taken_card.size == 0 and leave_card.size > 0
      ...
      leave_card.each do |card_l|
        ...
        pt_hands[card_l] -= @deck_info.get_card_rank(card_l)
      ...
      if points_cp >= 1 or @num_mani > 4
        ...
        taken_card.each do |card_t|
          ...
          pt_hands[card_t] += 15 - @deck_info.get_card_rank(card_t)
      ...
      @cards_on_hand.each do |k_suit, arrcards|
        ...
        arrcards.each do |card_ls|
          ...
          pt_hands[card_ls] -= 50 + @deck_info.get_card_rank(card_ls)
  ...
  def card_is_asso?(card)
    is_asso = @deck_info.get_card_logical_symb(card) == :asso
    #p " **** Card #{card} is asso? #{is_asso}" 
    return is_asso
  end
  
  def card_is_pezza?(card)
    is_pezza = @deck_info.get_card_points(card) == 1
  ...
  def card_can_take?(card_lbl, card_played)
    return @deck_info.get_card_rank(card_lbl) > @deck_info.get_card_rank(card_played) ? true : false
  end
  ...
  def onalg_manoend(player_best, carte_prese_mano, punti_presi)
    ...
    carte_prese_mano.each do |card|
      suit = @deck_info.get_card_segno(card)
  ...
  def put_cards_onhand(card)
    info = @deck_info.get_card_info(card)
  ...
  def onalg_player_has_played(player, card)
    ...
    card_played_segno = @deck_info.get_card_segno(card)
