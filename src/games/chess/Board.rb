...
class Board
  attr_reader :pieces, :color_on_turn, :last_moved_item, :all_available_moves, :moves_in_match
  ...
  def initialize
    ...
    @color_on_turn = nil 
    @log = Log4r::Logger.new("coregame_log::Board")
  ...
  def calculate_current_fen
    res_fen = ''
    @infosquare.each do |row|
      empty = 0
      row.each do |item|
        #p item.type_piece, item.to_string_piece, item.to_dest_pgn
        if item.type_piece != :vuoto
          if empty > 0
            res_fen.concat("#{empty}")
            empty = 0
          end
          str_piece = item.to_s_fenpiece
          res_fen.concat(str_piece)
        else
          empty += 1
        end
      end
      ...
      if row[0].row_start != 7
        res_fen.concat('/')
      end
    end
    res_fen.concat(' ')
    res_fen.concat(@color_on_turn == :black ? 'b' : 'w')
    res_fen.concat(' ')
    
    res_fen.concat(repr_castling)
    res_fen.concat(' ')
    
    res_fen.concat(repr_enpassant)
    res_fen.concat(' ')
    
    res_fen.concat(repr_fifty)
    res_fen.concat(' ')
    
    res_fen.concat(repr_fullmove)
  ...
  def repr_enpassant
    # anche qui non ho idea, forse ultima mossa ep?
    raise "Todo..."
  end
  ...
  def repr_fifty
    return "#{@fifty_counter}"
  end
  
  def repr_castling
    # TODO: fai una roba simile
    # 
  ...
  # una mossa valida se fa parte delle mosse possibili
  ...
  #def check_if_moveisvalid(argument)
    #color, start_x, start_y, end_x, end_y, movetype = strip_pos_argument(argument)
    #if !is_move_inside_the_board?(start_x, start_y, end_x, end_y) 
      #@last_move_error = {:code => 0x01, :msg => 'piece outside the board'}
      #return :invalid
    #end
    #if !is_player_on_turn?(color)
      #@last_move_error = {:code => 0x02, :msg => 'color not on turn'}
      #return :invalid
    #end
    #return :valid
  #end
  
  def strip_pos_argument(move)
    return move.color, move.row_s, move.col_s, move.row_e, move.col_e,move.movetype
  end
  
  def is_attacked(board_item)
    return false
  end
  
  def collect_all_moves
    @all_available_moves = []
    @infosquare.each do |row|
      row.each do |board_item|
        piece_color = board_item.color_piece
        if piece_color == @color_on_turn
          board_item.generate_possible_moves(self)
          #p board_item.possible_moves
          if board_item.possible_moves.size > 0
            @log.debug "Possible moves num: #{board_item.possible_moves.size}"
            board_item.print_all_moves
          end
          invalid_moves = []
          board_item.possible_moves.each do |m|
            do_the_move(m)
            list_king =  get_piece_list(:re, piece_color)
            if is_attacked(list_king.first)
              invalid_moves << m # mossa che mette sotto scacco il proprio re non vale
            else     
              @all_available_moves << m
            end
            pop_move
          end
        end
  ...
  def pop_move
    move = get_last_move
    return if move == nil
    @log.debug "pop_move #{move.move_to_str}"
    color, row_s, col_s, row_e, col_e, movetype = strip_pos_argument(move)
    if  movetype == :shortcastle
      # restore the king
      end_item = @infosquare[row_s][col_e - 2]
      start_item = @infosquare[row_s][col_e]
      BoardInfoItem.exchange(start_item, end_item)
      #restore the rook
      end_item = @infosquare[row_s][7]
      start_item = @infosquare[row_s][col_e - 1]
      BoardInfoItem.exchange(start_item, end_item)
      ...
      BoardInfoItem.exchange(start_item, end_item)
      #restore the rook
      end_item = @infosquare[row_s][0]
      start_item = @infosquare[row_s][col_e + 1]
      BoardInfoItem.exchange(start_item, end_item)
    elsif movetype == :move_eat
      end_item = @infosquare[row_s][col_s]
      start_item = @infosquare[row_e][col_e]
      BoardInfoItem.exchange(start_item, end_item)
      #restore eated piece
      start_item.setinfo(move.eated, get_opponent_color(move.color))
    elsif movetype == :move
      end_item = @infosquare[row_s][col_s]
      start_item = @infosquare[row_e][col_e]
      BoardInfoItem.exchange(start_item, end_item)
      ...
      start_item = @infosquare[row_e][col_e]
      BoardInfoItem.exchange(start_item, end_item)
      enp_item = @infosquare[row_s][col_e]
      #restore eated enpassant  piece
      enp_item.setinfo(move.eated, get_opponent_color(move.color))
    elsif movetype == :promotion
      end_item = @infosquare[row_s][col_s]
      start_item = @infosquare[row_e][col_e]
      BoardInfoItem.exchange(start_item, end_item)
      end_item.setinfo(:ped, move.color)
    else
      raise "Move #{movetype} not recognized"
    end
    
    @moves_in_match.pop
    
    actualized_last_move = get_last_move
    if actualized_last_move != nil
      color, row_s, col_s, row_e, col_e, movetype = strip_pos_argument(actualized_last_move)
      end_item = @infosquare[row_s][col_e]
      @last_moved_item = end_item
    else
      @last_moved_item = nil
    end
  ...
  def swap_color_on_turn
    @log.debug "Swap the color on turn"
  ...
  ##
  # Print the board
  def print_board(view_mode = :black)
    
    puts "Current chess board (#{view_mode} player view) black piece with *:"
    strline = ''
    board_lines = []
    count = 1
    if view_mode == :white
    ...
    row_arr = []
    if view_mode == :white
      @infosquare.reverse.each{|r| row_arr << r}
    else
      @infosquare.each{|r| row_arr << r}
    end
    row_arr.each do |row|
      line_coll = []
      if view_mode == :black
