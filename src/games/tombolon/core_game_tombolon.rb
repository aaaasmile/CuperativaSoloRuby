...
class CoreGameTombolon < CoreGameSpazzino
  ...
  def deck_table_isok?(mazzo_gioco)
    ...
    mazzo_gioco[0..3].each do |lbl_card|
      tot.each do |k, v|
        tot[k] += 1 if @deck_information.get_card_logical_symb(lbl_card) == k
      end
    end
    tot.each_value do |v|
      return false if v >= 3
  ...
  def points_scopa(card)
    pt = 0
    case @deck_information.get_card_logical_symb(card)
      when :fan
        pt = 3
      when :cav
        pt = 4
      when :re
        ...
        pt = @deck_information.get_card_rank(card)
    end
    return pt
  end
  ...
  def adjourn_points_manoend(player)
    ...
    @carte_prese[player.name].each do |card|
      ...
      if @deck_information.get_card_segno(card) == :S
        ...
        spade_arr << ix
        # check onori
        if @deck_information.get_card_logical_symb(card) == :fan
           points_info[:fantespade] = 1
        end
        if @deck_information.get_card_logical_symb(card) == :due
  ...
  def check_for_double_onori(hand)
    ...
    hand.each do |card|
      ...
      if @deck_information.get_card_logical_symb(card) == :fan and 
         @deck_information.get_card_segno(card) == :S
         fante_spade = true
      end
      if @deck_information.get_card_logical_symb(card) == :due and 
         @deck_information.get_card_segno(card) == :S
         due_spade = true
      end
    end
